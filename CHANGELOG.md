...
- Bump `eca-webview`: paint the select-box dropdown with `--eca-panel-bg` instead of `--eca-tooltip-bg`. IntelliJ New UI keeps `ToolTip.background` dark even on Light, which made the agent/variant/model popup unreadable on light themes. Option hover uses a `color-mix` overlay.
